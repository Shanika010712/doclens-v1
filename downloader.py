...
import requests # Import requests for HTTP errors
import re
import fitz
...
def check_for_files(sf_client,salesforce_id):
    ...
    try:
        ...
        logger.error(f"Salesforce authentication failed during check_for_files for ID {salesforce_id}: {e}", exc_info=True)
        raise # Re-raise to be caught by retry mechanism
    except requests.exceptions.RequestException as e:
        logger.error(f"Network/HTTP error during check_for_files for ID {salesforce_id}: {e}", exc_info=True)
        raise ConnectionError(f"Network error checking Salesforce files: {e}") from e
    except Exception as e:
        logger.error(f"Unexpected error during check_for_files for Salesforce ID {salesforce_id}: {e}", exc_info=True)
        raise ConnectionError(f"Unexpected error checking Salesforce files: {e}") from e
...
def Download_files(sf_client, DOWNLOAD_DIR, filtered_document_ids):
    ...
    """
    downloaded_paths = []
    downloaded_file_info = [] # List to store dicts of {'path': ..., 'sf_title': ...}
    ...
    for content_document_id in filtered_document_ids:
        try:
            ...
            # Sanitize the base name and get the extension
            base_name = sanitize_filename(version_data['Title'])
            extension = version_data['FileExtension']
            file_name = f"{base_name}.{extension}"
            file_path = os.path.join(DOWNLOAD_DIR, file_name)

            # Check for file existence and add a version suffix if needed
            counter = 1
            while os.path.exists(file_path):
                counter += 1
                new_file_name = f"{base_name}-v{counter}.{extension}"
                file_path = os.path.join(DOWNLOAD_DIR, new_file_name)
                logger.info(f"File '{file_name}' already exists. Saving new file as '{new_file_name}'.")
            file_name = os.path.basename(file_path) # Update file_name to the potentially new name
            ...
            logger.error(f"Salesforce authentication failed during Download_files for ID {content_document_id}: {e}", exc_info=True)
            raise # Re-raise to be caught by retry mechanism
        except requests.exceptions.RequestException as e:
            logger.error(f"Network/HTTP error during Download_files for ID {content_document_id}: {e}", exc_info=True)
            raise ConnectionError(f"Network error downloading Salesforce file {file_name}: {e}") from e
        except Exception as e:
...
def extract_salesforce_id(url):
    # Split the URL by '/' character
    parts = url.split('/')
    
    # Loop through the parts using enumerate
    for _, part in enumerate(parts):
        # Salesforce IDs are typically 18 characters long and follow a specific pattern
        # This pattern often includes uppercase letters and numbers
        if len(part) == 18 and part.isalnum():
            return part
...
def validate_link(link_input, link_type_from_user_selection):
    ...
    #salesforce_pattern = r"https?://([\w-]+\.)*(salesforce\.com|lightning\.force\.com).*"
    salesforce_pattern = r"https?://([\w-]+\.)*(salesforce\.com|lightning\.force\.com|my\.site\.com).*"
    #google_drive_pattern = r"https?://([\w-]+\.)*drive\.google\.com/(file/d/|drive/folders/|folderview\?id=).*"
    google_drive_pattern = r"https?://([\w-]+\.)*drive\.google\.com.*"
