...
def parse_structured_bank_details(raw_data_list_of_lists, bank_period_type, assessment_year):
    ...
    if len(general_info_raw) >= 6:
        general_info = {
            ...
            "nic": general_info_raw[2].strip() if isinstance(general_info_raw[2], str) else "",
    ...
    if bank_period_type == "Y": # Yearly
        ...
        for i, row in enumerate(raw_data_list_of_lists[1:], start=1): # Skip general info row
            ...
            if len(row) == 2 or len(row) == 3:
            ...
            elif len(row) == 4: # Period detail: [period_name, interest, wht, wht_cert_no]
                ...
                interest = row[1].strip() if isinstance(row[1], str) else "0"
                wht = row[2].strip() if isinstance(row[2], str) else "0"
                wht_cert_no = row[3].strip() if isinstance(row[3], str) else "" # Added wht_cert_no
                period_obj = {
...
def parse_balance_confirmation_output(pipe_string: str) -> dict | None:
    ...
    parts = [p.strip() for p in pipe_string.split('|')]
    
    if len(parts) < 5:
        logger.error(f"Insufficient data for general info in balance confirmation. Parts found: {len(parts)}")
        return None

    general_info = {
        "name_of_the_bank": parts[0],
        "name_of_the_person": parts[1],
        "nic_of_the_person": parts[2],
        "statement_date": parts[3],
        "year_of_assessment": parts[4],
    }

    accounts = []
    account_parts = parts[5:]
    # Each account has 8 fields
    for i in range(0, len(account_parts), 8):
        if i + 8 <= len(account_parts):
            account_chunk = account_parts[i:i+8]
            accounts.append(dict(zip(["account_number", "currency", "account_type", "balance", "interest", "wht", "is_joint", "joint_persons"], account_chunk)))
