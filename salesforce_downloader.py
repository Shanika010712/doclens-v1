...
def get_specific_document_title(sf_client, record_name):
    ...
    """
    print(f"  - Inside get_specific_document_title using system file name for record Name: {record_name}")
    query = f"""
    SELECT System_File_Name__c FROM Tick_Upload_File_names__c
    ...
    try:
        ...
        if result['totalSize'] > 0:
            ...
            print(f"  - Found specific title to process: '{title_without_extension}' (removed extension from '{full_filename}')")
            return title_without_extension, None
        return None, "No unprocessed documents found for this Tax File."
    except Exception as e:
        logger.error(f"Error in get_specific_document_title for record Name {record_name}: {e}", exc_info=True)
        return None, f"Error querying for the specific document title: {e}"
