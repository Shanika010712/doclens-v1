...
def create_case(sf_client, tax_file_id, subject, description, status='New', origin='AI Scan'):
    """
    ...
    try:
        ...
        if effective_doc.upper() == "EMPLOYMENT":
            ...
            if bank_period_type.upper() == "M":
                ...
                for acc_detail in accounts_from_payload:
                    # For yearly data, the account number is a key in the dictionary
                    acc_no = acc_detail.get("Account no.")                    
                    if not acc_no or not acc_no.strip(): # Ensure it's not None or an empty string
                        continue

                    # Populate user_accounts_details_list for Yearly
                    user_accounts_details_list.append([
                        acc_no,
                        acc_detail.get("Currency", "LKR"),
                        acc_detail.get("is_joint", False),
                        acc_detail.get("joint_persons", "")
                    ])

                    # Create a single-row DataFrame from the flattened account dictionary
                    df = pd.DataFrame([acc_detail])
                    bank_data_dfs[acc_no] = df
                main_record_success, main_record_message, main_record_id = bank_update_f(# Initialize user_accounts_details_list
                    sf_client, bank_data_dfs, general_bank_info_list, original_file_path,
                    request_user, assessment_year, user_accounts_details_list)
            else:
